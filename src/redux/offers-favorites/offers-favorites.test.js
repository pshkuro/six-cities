...
import {reducer, ActionType, ActionCreator, Operation} from "./offers-favotites.js";
...
describe(`Offers favorite reducer work correctly`, () => {
  ...
  it(`Should make a correct API post to /favorite`, () => {
    ...
    const mockId = 5;
    const mockStatus = 1;
    const favorite = Operation.setFavorite(mockId, mockStatus);

    apiMock
    .onPost(`/favorite/${mockId}/${mockStatus}`)
    .reply(204, mockFavoriteOffer);

    return favorite(dispatch, () => {}, api)
      .then(() => {
        expect(dispatch).toHaveBeenCalledTimes(1);
        expect(dispatch.mock.calls[0][0]).toEqual({type: ActionType.GET_FAVORITES, favorites: favoriteStateOffer});
      });
  });
});
